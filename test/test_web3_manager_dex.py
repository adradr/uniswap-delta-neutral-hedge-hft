...
def test_swap_amounts_no_swap_needed(web3_manager_dex):
    ...
    web3_manager_dex.update_wallet_balance = MagicMock(
        return_value=(
            web3_manager_dex.token0Balance,
            web3_manager_dex.token1Balance,
        )
    )
    web3_manager_dex.uniswap.get_current_price = MagicMock()
    web3_manager_dex.uniswap.swap_token_input = MagicMock()
...
def test_swap_amounts_token0_to_token1(web3_manager_dex):
    ...
    web3_manager_dex.tokenManager.token0_decimal = 6
    web3_manager_dex.tokenManager.token1_decimal = 18
    web3_manager_dex.uniswap.get_current_price = MagicMock(return_value=1000)
    web3_manager_dex.uniswap.swap_token_input = MagicMock(return_value="0x00")
    web3_manager_dex.uniswap.w3.eth.getTransactionReceipt = MagicMock(
        return_value=TxReceipt({"status": 1})
    )
...
def test_swap_amounts_insufficient_funds(web3_manager_dex):
    ...
    web3_manager_dex.token0Balance = 10**6 * 500  # token decimals 6 with 500 balance
    web3_manager_dex.token1Balance = 10**18 * 0  # token decimals 18 with 500 balance
    web3_manager_dex.amount0 = 10**6 * 1000  # required amount with token decimals 6
    web3_manager_dex.amount1 = 10**18 * 1  # required amount with token decimals 18
    web3_manager_dex.tokenManager.token0_decimal = 6
    web3_manager_dex.tokenManager.token1_decimal = 18
    web3_manager_dex.uniswap.get_current_price = MagicMock(return_value=1000)

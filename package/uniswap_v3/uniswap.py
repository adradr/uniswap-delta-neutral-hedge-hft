...
class Uniswap:
    ...
    def __init__(
        ...
        if self.version != 3:
            raise Exception(
                f"Invalid version '{self.version}', only version 3 is supported"
            )  # pragma: no cover
    ...
    def _get_token_eth_output_price(
        self, token: AddressLike, qty: Wei, fee: Union[int, None] = None  # input token
    ) -> int:
    ...
    # ------ Make Trade ----------------------------------------------------------------
    @check_approval
    def check_approval_and_make_trade(
        self,
        input_token: AddressLike,
        output_token: AddressLike,
        qty: Union[int, Wei],
        recipient: Union[AddressLike, None] = None,
        fee: Union[int, None] = None,
    ...
    def _eth_to_token_swap_output(
        ...
        if self.version == 1:
            ...
            return self._build_and_send_tx(
                ...
                self._get_tx_params(eth_qty),  # type: ignore
            )
        elif self.version == 3:
            if recipient is None:
                recipient = self.address
    ...
    @check_approval
    @supports([3])
    def check_approval_and_mint_liquidity(
        self,
        pool: Contract,
        amount_0: int,
        amount_1: int,
        ...
        *_, isInit = pool.functions.slot0().call()
        ...
        # TODO: add slippage param
        tx_hash = self.nonFungiblePositionManager.functions.mint(
            (
                token_0,
                token_1,
                fee,
    ...
    def _is_approved(self, token: AddressLike) -> bool:
        ...
        amount = (
            ...
            .functions.allowance(self.address, contract_addr)  # type: ignore
            .call()
        )
    ...
    def _build_and_send_tx(
        ...
        try:
            ...
            logger.debug(f"nonce: {tx_params['nonce']}")  # type: ignore
            self.last_nonce = Nonce(tx_params["nonce"] + 1)  # type: ignore

    def _get_tx_params(
        self, value: Wei = Wei(0), gas: Union[Wei, None] = None
    ) -> TxParams:
    ...
    def _exchange_contract(
        self,
        token_addr: Union[AddressLike, None] = None,
        ex_addr: Union[AddressLike, None] = None,
    ) -> Contract:
